#ifndef UTILITIES_H
#define UTILITIES_H

#include <iostream>
#include <string>
#include <algorithm>
#include <cctype>

using namespace std;

class tourista_utils {
public:
    // 1. DATA SAFETY: Prevents 'stoi' crashes on empty/corrupt strings
    static int safe_stoi(const string& s) {
        if (s.empty()) return 0;
        
        // Remove potential hidden whitespace/newlines
        string trimmed = s;
        trimmed.erase(trimmed.find_last_not_of(" \n\r\t") + 1);

        if (trimmed.empty()) return 0;

        // Ensure every character is a digit
        for (char const &c : trimmed) {
            if (!isdigit(c)) return 0;
        }

        try {
            return stoi(trimmed);
        } catch (...) {
            return 0;
        }
    }

    // 2. FILE SYSTEM: Centralized Path Management
    static string get_path() {
        return "DATA/"; 
    }

    // 3. SECURITY: Email Validation
    static bool is_valid_email(string email) {
        size_t at_pos = email.find('@');
        size_t dot_pos = email.find(".com");
        return (at_pos != string::npos && dot_pos != string::npos && dot_pos > at_pos);
    }

    // 4. UTILITY: Case-Insensitive String Conversion
    static string to_lower(string data) {
        string lower_data = data;
        transform(lower_data.begin(), lower_data.end(), lower_data.begin(), ::tolower);
        return lower_data;
    }
};

#endif